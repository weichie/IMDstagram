<section class="post">
	<header>
		<h5>
			<a href="#!">
				<img src="<?php htmlspecialchars($post['avatar'], ENT_QUOTES, 'UTF-8'); ?>" />
				<?php htmlspecialchars($post['username'], ENT_QUOTES, 'UTF-8'); ?>
			</a>
			<span class="pull-right"><?=$this->ago(strtotime($post['date']))?></span>
			<a href="<?=SITE_URL?>/?route=user/deletePost&id=<?php htmlspecialchars($post['post_id'], ENT_QUOTES, 'UTF-8'); ?>" class="pull-right">foto verwijderen?</a>
		</h5>
	</header>
			...
			<ul>
				<?php if( !empty($post['likes']) ): ?>
						<?php foreach($post['likes'] as $like): ?>
							<li>
								<a href="#!"><?php htmlspecialchars($like['username'], ENT_QUOTES, 'UTF-8'); ?></strong> <?= $this->linkHashtags($like['like']); ?>
							</li>
						<?php endforeach; ?>
				<?php endif; ?>
			</ul>
